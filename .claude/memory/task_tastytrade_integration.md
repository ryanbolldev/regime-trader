...
**Parity — GATE NOT YET PASSED (but largely de-risked).** `scripts/tastytrade_parity_check.py` diffs Alpaca vs tastytrade `WheelOptionLeg` by (expiration, strike); enhanced to report signed bias (mean tt−alpaca), Pearson corr, and an IV-gap breakdown by delta bucket. Two **after-hours** MSTR runs (21-45 DTE):
- delta: median|Δ|≈0.007, corr≈0.999 → parity ✓
- bid/ask: corr≈1.000 (large max|Δ| = after-hours wide spreads) ✓
- open_interest: Alpaca returns None → uncomparable (the whole reason for tastytrade)
- **IV: the earlier scary "0.19" was dominated by ITM puts.** By delta bucket: deep-OTM 0.036, **OTM .15–.35 (the wheel's zone, target delta −0.28) ≈0.044 with a consistent −0.045 bias**, ATM 0.065, **ITM 0.16**. The wheel never sells ITM puts, so that noise is irrelevant. Low overall corr (0.31) is dragged down by ITM.

**Interpretation:** in the wheel's operating region the IV disagreement is a small *consistent downward* offset (tastytrade ~0.045 lower), not random noise. Since `compute_ivr` uses the same realized-vol range for both providers, this shifts IV Rank predictably (tastytrade reads slightly lower → marginally fewer names clear `WHEEL_MIN_IV_RANK=40`) — tunable, not broken.

**MARKET-HOURS RUN DONE (2026-07-07 08:35 CT) — GATE CLEARED, pending flip sign-off.** Task `RegimeTrader-ParityCheck` fired successfully (exit 0) → `logs/parity_check_2026-07-07.txt`. Result (MSTR, 304 matched): delta median|Δ|=0.017 corr=.998 ✓; **IV gap collapsed vs after-hours** — wheel zone **OTM .15–.35 now median|Δ|=0.016, bias=−0.021** (was 0.044/−0.045), ITM 0.16→0.053. So most of the scary gap was staleness. Small residual ~−0.02 (tastytrade reads ~2 vol pts lower → makes `WHEEL_MIN_IV_RANK=40` slightly stricter, safe direction). **Alpaca still returned open_interest=None during market hours → confirms the core rationale for tastytrade.** **FLIPPED 2026-07-07 (Ryan signed off):** `OPTIONS_DATA_PROVIDER="tastytrade"`, kept `WHEEL_MIN_IV_RANK=40` (small residual makes gate slightly stricter — safe). Full suite 1013 passed. tastytrade is now the live options-data source (incl. OI). Spent one-time scheduler task removed. **tastytrade integration COMPLETE.**

Note: original wrapper mojibaked box/greek glyphs (PowerShell re-decoded Python UTF-8 via OEM codepage) — fixed to use `cmd` byte-redirect; today's file data was intact regardless. Decision rule: if the OTM-bucket bias shrinks toward 0 → staleness → flip cleanly; if the ~−0.045 OTM bias holds → methodology → flip and either accept it or nudge `WHEEL_MIN_IV_RANK` down ~2 pts. Must run LOCALLY (needs .env creds + OS trust store; see [[env_tls_proxy]]). Human-sign-off verification gate (data-source change) before flipping `OPTIONS_DATA_PROVIDER`.
